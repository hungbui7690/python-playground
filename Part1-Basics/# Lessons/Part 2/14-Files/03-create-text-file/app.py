...
# 1) For example, the following creates a new file called readme.txt and write some text into it:
with open(".\\playground\\readme.txt", "w") as f:
    f.write("Create a new text file!")


# 2) This script creates a file with the name readme.txt in the same directory where the script file locates. If you want to create a file in a specified directory e.g., docs/readme.text, you need to ensure that the docs directory exists before creating the file. Otherwise, you’ll get an exception. For example:
...
# 3) Also, you can handle the exception using the try-except statement as follows:
# try:
#     with open("docs/readme.txt", "w") as f:
#         f.write("Create a new text file!")
...
# 4) If you don’t want to create a new text file in case it already exists, you can use the 'x' mode when calling the open() function:
try:
    with open("readme.txt", "x") as f:
        f.write("Create a new text file!")
except FileExistsError:
    print("Error: File exists in current directory")
